...
| Package | Description |
|---------|-------------|
| `@erdo/server` | Server-side client for invoking Erdo agents |
| `@erdo/ui` | React components for rendering agent results |
...
```bash npm
npm install @erdo/server @erdo/ui
```

```bash yarn
yarn add @erdo/server @erdo/ui
```

```bash pnpm
pnpm add @erdo/server @erdo/ui
```
...
```tsx
import { ErdoProvider, Content, useInvocation } from '@erdo/ui';
import { ErdoClient } from '@erdo/server';
