...
### Session Tokens (B2B2C)

For B2B2C applications where your customers need to expose Erdo datasets to their end users, use session tokens. Session tokens are:

- **Short-lived**: Expire after 1-24 hours (configurable)
- **Scoped**: Only grant access to specific datasets
- **Revocable**: Can be revoked by your backend at any time

This pattern is ideal for:
- SaaS products embedding data visualizations for their customers
- Dashboards where end users should only see specific datasets
- Applications requiring fine-grained, temporary access control

#### Creating Session Tokens (Backend)

Create a session token from your backend using your API key:
...
app.post('/api/erdo-session', async (req, res) => {
  // Get the datasets this user should have access to
  const datasetIds = await getUserDatasets(req.user.id);

  // Create a session token scoped to those datasets
  const response = await fetch('https://api.erdo.ai/dataset-sessions', {
    method: 'POST',
    headers: {
      'Authorization': `Bearer ${process.env.ERDO_API_KEY}`,
      'Content-Type': 'application/json',
    ...
    body: JSON.stringify({
      dataset_slugs: datasetIds,
      external_user_id: req.user.id,  // Optional: for tracking
...
#### Using Session Tokens (Frontend)

Pass the session token to `ErdoProvider`:

```tsx
'use client';

import { ErdoProvider, DatasetChart } from '@erdo/ui';
...
function Dashboard() {
  ...
  useEffect(() => {
    ...
    fetch('/api/erdo-session', { method: 'POST' })
      .then(res => res.json())
  ...
  return (
    <ErdoProvider
      config={{
        ...
        sessionToken,  // Use session token instead of authToken
      }}
    >
      <DatasetChart
        chartType="bar"
...
Revoke a session when users log out or when access should be removed:

```typescript
// Your backend
app.post('/api/erdo-session/revoke', async (req, res) => {
  await fetch('https://api.erdo.ai/dataset-sessions/revoke', {
    method: 'POST',
    headers: {
      'Authorization': `Bearer ${process.env.ERDO_API_KEY}`,
      'Content-Type': 'application/json',
...
#### Session Token API Reference

| Endpoint | Method | Description |
|----------|--------|-------------|
| `/dataset-sessions` | POST | Create a new session token |
| `/dataset-sessions` | GET | List active sessions |
| `/dataset-sessions/revoke` | POST | Revoke a session |
...
```json
{
  "session_token": "erdo_session_abc123...",
  "session_id": "uuid",
  "expires_at": "2024-01-01T12:00:00Z",
  "scopes": [
    { "dataset_id": "uuid", "dataset_slug": "dataset-1", "access_level": "read" }
  ]
}
```

<Note>
Session tokens are more secure than API keys for client-side use because they:
- Expire automatically
- Only grant access to specific datasets
- Can be revoked immediately when needed
</Note>
