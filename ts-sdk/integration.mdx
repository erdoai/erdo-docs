...
const client = new ErdoClient({ authToken: process.env.ERDO_API_KEY });

export async function POST(request: Request) {
  const { botKey } = await request.json();

  // Add your own RBAC logic here
  // if (!user.canAccess(botKey)) return Response.json({ error: 'Forbidden' }, { status: 403 });
...
export default function ChatPage() {
  ...
  const getToken = useCallback(async (botKey: string) => {
    ...
    tokenRef.current = { token: data.token, expiresAt: new Date(data.expiresAt) };
    return data.token;
  }, []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const token = await getToken('data-analyst');
...
export default function ChatPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Parse SSE stream from your proxy
    const reader = response.body?.getReader();
    const decoder = new TextDecoder();
    let buffer = '';
