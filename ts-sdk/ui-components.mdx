...
Components for rendering streaming invocation results.
...
```tsx
import { InvocationEvents } from '@erdoai/ui';

<InvocationEvents
  eventsByID={invocation.eventsByID}
  ContentComponent={Content} // Optional - uses provider default
/>
```

### InvocationEvent

Routes a single event to the appropriate renderer:

```tsx
import { InvocationEvent } from '@erdoai/ui';
...
Renders output contents from a step:

```tsx
import { Output } from '@erdoai/ui';
...
Renders a step with its status and nested events:

```tsx
import { StepInvocation } from '@erdoai/ui';
...
<Note>
All invocation components use `ContentComponent` from the `ErdoProvider` context by default. You only need to pass it explicitly if you want to override the provider's default.
</Note>
...
```tsx
import { useInvocation } from '@erdoai/ui';
