...
<AccordionGroup>
  ...
  <Accordion title="Result Handlers">
    ...
    @automation_step.when(IsSuccess() & GreaterThan("confidence", "0.8"))
...
# Handle high-complexity tasks with decorator
@analyze_step.when(IsSuccess() & GreaterThan("complexity", "0.8"))
def handle_complex_task(result):
    """Store complex tasks for detailed analysis."""
    return memory.store(memory={
...
- **Result handlers** use `step.on()` or `@step.when()` for conditional logic
