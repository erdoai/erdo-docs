...
Use `@step.when()` decorators for conditional result handling:
...
@primary_step.when(IsError())
def fallback_processing(error):
...
@initial_check.when(IsSuccess() & TextContains("assessment", "complex"))
def run_detailed_analysis(result):
...
@initial_check.when(IsSuccess() & TextContains("assessment", "simple"))
def run_basic_analysis(result):
...
@analysis_step.when(IsSuccess() & GreaterThan("confidence", "0.8"))
def store_high_quality_results(result):
    return memory.store(memory={
