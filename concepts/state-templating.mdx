...
1. **Start with simple fields**: Convert basic `"{{field}}"` to `state.field` for direct value access
2. **Use TemplateString for dynamic content**: Replace string templates with `TemplateString("{{field}}")`
3. **Keep complex templates**: Use `TemplateString` for complex expressions and functions
