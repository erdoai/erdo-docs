...
analysis_step.on(
    ...
    memory.store(memory={
        ...
        "extra": {
            "recommendations": "{{analysis_step.output.recommendations}}",
            "key_findings": "{{analysis_step.output.key_findings}}"
        }
    })
)
...
stats_step.on(
    IsSuccess() & GreaterThan("growth_rate", 0),
    utils.send_status(
        status="positive_growth",
        message="Excellent! Sales showing {{stats_step.output.growth_rate}} growth rate",
        priority="high"
    )
)
...
preprocess_step.on(
    ...
    utils.send_status(
        status="data_error",
        message="Data preprocessing failed: {{preprocess_step.error.message}}",
        error_details="{{preprocess_step.error.details}}"
    )
)
...
<CardGroup cols={2}>
  ...
  >
    Explore business process automation examples
  </Card>
  ...
  <Card title="Pre-built Agents" icon="robot" href="/agents/data-analyst">
    Leverage ready-made data analysis solutions
  </Card>
</CardGroup>
