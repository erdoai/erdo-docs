...
response_step.on(
    ...
    utils.send_status(
        ...
        response_content="{{response_step.output.response}}"
    )
)
...
response_step.on(
    ...
    memory.store(memory={
        "content": "Complex support case: {{response_step.output.analysis}}",
        "type": "escalated_support",
        "tags": ["support", "escalated", "{{email_sender}}"],
        "extra": {
            "original_email": "{{email_content}}",
            "suggested_response": "{{response_step.output.response}}",
...
response_step.on(
    IsSuccess(),
    memory.store(memory={
        "content": "Email from {{email_sender}}: {{email_content}}\nResponse: {{response_step.output.response}}",
        "type": "email_interaction",
        "tags": ["email", "{{classify_step.output.category}}", "{{email_sender}}"],
        "searchable_texts": ["{{email_content}}", "{{response_step.output.response}}"]
    })
)
```
...
analysis_step.on(
    ...
    memory.store(memory={
        "content": "Complex document requiring review: {{analysis_step.output.document_type}}",
        "type": "document_review_required",
        "tags": ["document", "review", "complex"],
        "extra": {
            "document_type": "{{analysis_step.output.document_type}}",
            "complexity": "{{analysis_step.output.processing_complexity}}",
...
validation_step.on(
    IsSuccess() & TextEquals("is_valid", "true"),
    memory.store(memory={
        ...
        "extra": {
            "validation_score": "{{validation_step.output.completeness_score}}",
            "extracted_fields": "{{extraction_step.output.extracted_data}}"
        }
    })
)

# Flag documents with validation issues
validation_step.on(
    IsSuccess() & TextEquals("is_valid", "false"),
    utils.send_status(
        status="validation_failed",
        message="Document validation failed - manual review required",
        validation_errors="{{validation_step.output.errors}}"
    )
)
```
...
risk_assessment.on(
    ...
    utils.send_status(
        ...
        priority="urgent",
        risk_factors="{{risk_assessment.output.risk_factors}}",
        invoice_data="{{extract_step.output}}"
    )
)
...
extract_step.on(
    IsSuccess(),
    memory.store(memory={
        "content": "Invoice processed: {{extract_step.output.invoice_number}} from {{extract_step.output.vendor_name}}",
        "type": "invoice_audit_trail",
        "tags": ["audit", "invoice", "{{extract_step.output.vendor_name}}"],
        "extra": {
            "invoice_data": "{{extract_step.output}}",
...
document_analysis.on(
    IsSuccess() & LessThan("completeness_score", "0.8"),
    utils.send_status(
        status="additional_documents_required",
        message="Additional documentation required to complete onboarding",
        missing_documents="{{document_analysis.output.missing_documents}}",
...
compliance_check.on(
    IsSuccess() & ~TextEquals("overall_compliance", "approved"),
    memory.store(memory={
        "content": "Compliance issues detected for customer {{customer_info.id}}",
        "type": "compliance_flag",
        "tags": ["compliance", "onboarding", "{{customer_info.id}}"],
        "extra": {
            "compliance_status": "{{compliance_check.output.overall_compliance}}",
            "required_actions": "{{compliance_check.output.required_actions}}",
            "aml_status": "{{compliance_check.output.aml_status}}",
            "kyc_status": "{{compliance_check.output.kyc_status}}"
        }
    })
)
...
welcome_package.on(
    ...
    utils.send_status(
        ...
        customer_id="{{customer_info.id}}",
        welcome_content="{{welcome_package.output.welcome_message}}"
    )
)
...
step.on(
    ...
    utils.send_status(
        ...
        message="Processing failed: {{step.error.message}}"
    )
)
```
...
analysis_step.on(
    IsSuccess() & GreaterThan("amount", "10000"),
    utils.send_status(
...
analysis_step.on(
    IsSuccess() & TextContains("category", "international"),
    utils.send_status(
...
step.on(
    IsSuccess(),
    memory.store(memory={
        "content": "Operation completed: {{step.output.operation_type}}",
        "type": "audit_record",
        "tags": ["audit", "{{operation_type}}", "{{user_id}}"],
        "extra": {
            "operation_details": "{{step.output}}",
