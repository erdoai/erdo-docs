...
<CardGroup cols={2}>
  <Card title="Python Script" icon="code">
    ```bash erdo export-bot my-agent --format python ``` Single Python file with
    all code
  </Card>
  <Card title="ZIP Archive" icon="archive">
    ```bash erdo export-bot my-agent --format zip --include-deps ``` Complete
    project structure
  </Card>
  <Card title="Docker Image" icon="docker">
    ```bash erdo export-bot my-agent --format docker --tag my-agent:v1.0 ```
    Containerized deployment
  </Card>
  <Card title="YAML Config" icon="file-code">
    ```bash erdo export-bot my-agent --format yaml --config-only ```
