...
<Tabs>
  <Tab title="CLI">
    ```bash
    # Test agent locally
    erdo test my-agent --input '{"query": "analyze data"}'
    
    # Run agent with parameters
    erdo agent run my-agent --param dataset=sales.csv
    
    # Interactive testing
    erdo agent test my-agent --interactive
    ```
  </Tab>
  <Tab title="SDK">
    ```python
    ...
    # Invoke agent object directly
    result = Invoke(agent, parameters={"dataset": "sales.csv"})
    
    # Streaming mode
    result = Invoke.by_key(
        "erdo.my-agent", 
        parameters={"query": "large analysis"},
        stream=True
    )
...
<Tabs>
  <Tab title="CLI">
    ...
    erdo test my-agent --test-data test_data.json
    ```
  </Tab>
  <Tab title="SDK">
    ```python
    ...
    simulation_results = test.simulate(data={"input": "test"})
    
    # Live testing via platform
    live = LiveTest()
    results = live.run_all()
    results = live.run_by_agent("my-agent")
    ```
  </Tab>
</Tabs>
