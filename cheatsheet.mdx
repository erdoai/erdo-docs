...
analyze_step = agent.step(
    llm.message(model="claude-sonnet-4-5-20250514", user_prompt="{{query}}"),
    key="analyze",
)
...
analyze = agent.step(
    llm.message(model="claude-sonnet-4-5", user_prompt="Analyze: {{steps.search.output}}"),
    key="analyze",
    depends_on=search,
)
report = agent.step(
    llm.message(model="claude-sonnet-4-5", user_prompt="Create report: {{steps.analyze.output}}"),
    key="report",
    depends_on=analyze,
)
```
