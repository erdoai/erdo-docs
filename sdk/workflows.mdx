...
@analyze_data_step.when(IsSuccess() & LessThan("data_size", 1000) & TextContains("complexity", "low"))
def simple_processing(result):
    return codeexec.execute(
        code="simple_data_processing({{input_data}})"
    )

# Advanced processing for large or complex datasets
@analyze_data_step.when(IsSuccess() & (GreaterThan("data_size", 1000) | TextContains("complexity", "high")))
def advanced_processing(result):
    return codeexec.execute(
...
@classify_step.when(TextContains("content_type", "text"))
...
@classify_step.when(TextContains("content_type", "image"))
def process_image(result):
    return codeexec.execute(
        code="process_image_content({{content}})"
    )

@classify_step.when(TextContains("content_type", "document"))
...
@main_processing.when(IsSuccess())
...
@main_processing.when(IsError() & LessThan("retry_count", 3) & TextContains("error", "timeout"))
def retry_on_timeout(error):
    return utils.echo(
        data={
...
@main_processing.when(IsError() & TextContains("error", "syntax"))
def regenerate_code(error):
    return llm.message(
        model="claude-sonnet-4",
...
@main_processing.when(IsError() & GreaterThan("retry_count", 2))
