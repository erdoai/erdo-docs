...
class InvokeResult:
    ...
    result: Optional[Any]           # Formatted result (depends on output_format)
...
if response.success:
    ...
    # Get invocation ID
    print(f"Invocation: {response.invocation_id}")
...
assert text_contains(str(response.result), "expected output")
```
