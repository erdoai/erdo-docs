...
# Add result handlers with decorators
@response_step.when(IsSuccess() & GreaterThan("confidence", "0.8"))
def store_high_quality_response(result):
...
- **Decorator-based handlers**: Use `@step.when()` for result handling
...
### Decorator Handlers with @step.when()
...
@step.when(IsSuccess())
def handle_success(result): pass

@step.when(IsError())
def handle_error(error): pass

# Numeric comparisons
@step.when(IsSuccess() & GreaterThan("score", "0.8"))
def handle_high_score(result): pass

# Text conditions
@step.when(IsSuccess() & TextContains("category", "urgent"))
def handle_urgent(result): pass
```
...
def handle_complex_condition(result):
    return memory.store(memory={
...
# Handle successful execution with decorator
@execute_step.when(IsSuccess() & ~TextContains("stderr", "error"))
...
# Handle errors
@execute_step.when(IsError())
def handle_execution_error(error):
    return utils.send_status(
        status="failed",
        message=f"Execution failed: {error.message}"
    )
```
